...
async def parse_cmd(argv: Optional[Sequence[str]] = None):
    ...
    def main(
        ...
        poll_latest: Annotated[
            str,
            typer.Option("--poll_latest", help="是否开启无限轮询最新", rich_help_panel="基础配置"),
        ] = str(getattr(config, "ENABLE_POLL_LATEST", False)),

        poll_interval: Annotated[
            int,
            typer.Option("--poll_interval", help="轮询间隔秒数", rich_help_panel="基础配置"),
        ] = 0,

        search_pages: Annotated[
            int,
            typer.Option("--search_pages", help="每轮抓取页数（抖音每页10条）", rich_help_panel="基础配置"),
        ] = getattr(config, "SEARCH_PAGES", 2),

        kafka_json_dir: Annotated[
            str,
            typer.Option("--kafka_json_dir", help="Kafka JSONL 输出目录", rich_help_panel="存储配置"),
        ] = "",
        ...
        # 这些你新增的功能：始终覆盖（有默认值就行）
        config.ENABLE_POLL_LATEST = _to_bool(poll_latest)
        config.SEARCH_PAGES = search_pages

        # ✅ 只有在命令行真的传了值时才覆盖（否则留给 main.py 交互输入）
        ...
        if kafka_json_dir and kafka_json_dir.strip():
            config.KAFKA_JSON_DIR = kafka_json_dir.strip()
        ...
        return SimpleNamespace(
            ...
            poll_latest=config.ENABLE_POLL_LATEST,
            poll_interval=config.POLL_INTERVAL_SEC,
            search_pages=config.SEARCH_PAGES,
            kafka_json_dir=config.KAFKA_JSON_DIR,
        )
