...
class BilibiliCrawler(AbstractCrawler):
    ...
    async def search(self):
        ...
        if config.BILI_SEARCH_MODE == "normal":
            if getattr(config, "ENABLE_POLL_LATEST", False):
                await self.search_poll_latest_two_pages()
            else:
    ...
    async def search_poll_latest_two_pages(self):
        """
        B站：无限轮询抓最新（每轮前 N 页，默认 2），只处理新增视频，持续写入
        """
        utils.logger.info("[BilibiliCrawler.search_poll_latest_two_pages] Poll latest enabled. Press Ctrl+C to stop.")

        page_size = 20  # B站每页固定 20
        pages = int(getattr(config, "SEARCH_PAGES", 2) or 2)
        poll_interval = int(getattr(config, "POLL_INTERVAL_SEC", 30) or 30)

        keywords = [k.strip() for k in config.KEYWORDS.replace("，", ",").split(",") if k.strip()]
        if not keywords:
            utils.logger.warning("[BilibiliCrawler.search_poll_latest_two_pages] No keywords provided.")
            return

        seen_aids = set()  # 本次运行内去重（视频 aid）

        async def fetch_keyword_once(keyword: str):
            source_keyword_var.set(keyword)
            new_video_aids: List[str] = []

            # 查前 N 页：page=1..N，按最新发布排序
            for page in range(1, pages + 1):
                utils.logger.info(f"[BilibiliCrawler.poll] keyword={keyword} page={page}/{pages}")
                ...
                try:
                    videos_res = await self.bili_client.search_video_by_keyword(
                        ...
                        page=page,
                        page_size=page_size,
                        order=SearchOrderType.LAST_PUBLISH,  # ✅ 最新
                        pubtime_begin_s=0,
                        pubtime_end_s=0,
                    )
                except Exception as e:
                    utils.logger.warning(f"[BilibiliCrawler.poll] search failed keyword={keyword} page={page}: {e}")
                    continue
                ...
                # 批量拿详情（沿用原项目逻辑）
                semaphore = asyncio.Semaphore(config.MAX_CONCURRENCY_NUM)
                tasks = [self.get_video_info_task(aid=aid, bvid="", semaphore=semaphore) for aid in aids]
                ...
                for video_item in video_items:
                    if not video_item:
                        continue
                    try:
                        aid_val = video_item.get("View", {}).get("aid")
                        if aid_val:
                            new_video_aids.append(aid_val)
                        await bilibili_store.update_bilibili_video(video_item)
                        await bilibili_store.update_up_info(video_item)
                        await self.get_bilibili_video(video_item, semaphore)
                    except Exception as e:
                        utils.logger.warning(f"[BilibiliCrawler.poll] process video failed: {e}")



            # 抓评论（只对新增视频）
            if new_video_aids:
                await self.batch_get_video_comments(new_video_aids)
                utils.logger.info(f"[BilibiliCrawler.poll] keyword={keyword} new videos={len(new_video_aids)} -> comments done")
            else:
                utils.logger.info(f"[BilibiliCrawler.poll] keyword={keyword} no new videos")
