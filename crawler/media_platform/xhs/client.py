...
class XiaoHongShuClient(AbstractApiClient):
    ...
    async def request(self, method, url, **kwargs) -> Union[str, Any]:
        ...
        if response.status_code in (471, 461):
            # 有时会被风控/验证码拦截，但响应头不一定带 Verifytype/Verifyuuid
            verify_type = response.headers.get("Verifytype") or response.headers.get("verifytype")
            verify_uuid = response.headers.get("Verifyuuid") or response.headers.get("verifyuuid")

            msg = (
                f"出现验证码/风控拦截，请求失败。"
                f"status={response.status_code}, "
                f"Verifytype={verify_type}, Verifyuuid={verify_uuid}, "
                f"headers_keys={list(response.headers.keys())}, "
                f"body_head={response.text[:200]!r}"
            )

            # 这里用项目自带 logger（通常是 utils.logger）
            utils.logger.error(msg)

            # 抛出项目里已有的异常类型，外层会触发重试/重新登录
